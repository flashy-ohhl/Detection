...
model = dict(
    roi_head=dict(
        type='PETDetRoIHead',
        contrast_level='proposal',   # proposal-level object-anchored contrast
        num_aug=3,                   # K (adversary output size)
        max_pos_per_img=256,         # cap positives per image (memory)
        contrastive_loss=dict(
            type='BidirectionalContrastiveLoss',
            temperature=0.07,
            loss_weight_disc=0.5,
            loss_weight_nuis=0.5),
        # GRL disentanglement: strip aug info from F_disc, class info from F_nuis
        disentangle_adv=dict(
            enable=True,
            hidden_channels=128,
            grl_alpha=1.0,
            grl_warmup_iters=2000,
            loss_weight_disc=0.1,
            loss_weight_nuis=0.1),
        bbox_head=dict(
